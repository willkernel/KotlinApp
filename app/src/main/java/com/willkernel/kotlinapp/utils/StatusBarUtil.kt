...
class StatusBarUtil {
    ...
    companion object {
        ...
        @JvmOverloads
        fun immersive(activity: Activity, color: Int = DEFAULT_COLOR, alpha: Float = DEFAULT_ALPHA) {
        ...
        @JvmOverloads
        fun immersive(window: Window, color: Int, alpha: Float = 1f) {
...
//<editor-fold desc="DarkMode">
        ...
        /** 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上)  */
        @TargetApi(Build.VERSION_CODES.M)
        fun darkMode(window: Window, color: Int, alpha: Float) {
            when {
                isFlyme4Later -> {
                ...
                isMIUI6Later -> {
                    Log.e(TAG, "isMIUI6Later $isMIUI6Later")
...
//            }
        ...
        fun darkModeForMIUI6(window: Window, darkmode: Boolean): Boolean {
            ...
            return try {
                ...
                Log.e(TAG, "darkModeFlag $darkModeFlag")
                Log.e(TAG, "extraFlagField $extraFlagField")
                true
            } catch (e: Exception) {
                Log.e(TAG, "darkModeForMIUI6 ${e.printStackTrace()}")
                e.printStackTrace()
                false
            }
         ...
         * 创建假的透明栏
         */
        fun setTranslucentView(container: ViewGroup, color: Int, alpha: Float) {
        ...
        fun mixtureColor(color: Int, alpha: Float): Int {
